...
class MAMLAlgo(MetaAlgo):
    ...
    def __init__(self, policy, inner_lr=0.1, meta_batch_size=20, num_inner_grad_steps=1,
    ...
    def _make_input_placeholders(self, prefix=''):
        ...
        for task_id in range(self.meta_batch_size):
            # observation ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.policy.obs_dim],
                                name='obs' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'observations')] = ph
            obs_phs.append(ph)

            # action ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.policy.action_dim],
            ...
            for info_key, shape in dist_info_specs:
                ph = tf.placeholder(dtype=tf.float32, shape=[None] + list(shape),
